...
import 'package:smatch/callclub/widgets/common/app_bar/meeting_appbar.dart';
import 'package:smatch/callclub/widgets/common/joining/participant_limit_reached.dart';
import 'package:smatch/callclub/widgets/common/joining/waiting_to_join.dart';
import 'package:smatch/callclub/widgets/common/participant/participant_list.dart';
import 'package:smatch/callclub/widgets/one-to-one/one_to_one_meeting_container.dart';
...
class _OneToOneMeetingScreenState extends State<OneToOneMeetingScreen> {
  ...
  void initState() {
    ...
    Room room = VideoSDK.createRoom(
      ...
      notification: const NotificationInfo(
        title: "Smatch",
        message: "Smatch partage l'écran dans le salon",
                          ...
                          onMoreOptionSelected: (option) {
                            ...
                            if (option == "screenshare") {
                              if (remoteParticipantShareStream == null) {
                                ...
                                showSnackBarMessage(
                                    message: "Quelqu'un présente déjà",
                                    context: context);
                              }
                              ...
                              if (recordingState == "STARTED") {
                                ...
                                showSnackBarMessage(
                                    message:
                                        "L'enregistrement est en état de démarrage",
                                    context: context);
                              } else {
                                setState(() {
  ...
  void registerMeetingEvents(Room _meeting) {
    ...
    _meeting.on(Events.roomLeft, (String? errorMsg) {
      if (errorMsg != null) {
        showSnackBarMessage(
            message: "Réunion annulée en raison de $errorMsg !!",
            context: context);
      }
      Navigator.pushAndRemoveUntil(
          ...
          MaterialPageRoute(
              builder: (context) => JoinScreen(
                    nameuser: _meeting.localParticipant.displayName,
                    idbranche: '',
                  )),
          (route) => false);
    });
    ...
    _meeting.on(Events.recordingStarted, () {
      showSnackBarMessage(
          message: "L'enregistrement de la réunion a commencé",
          context: context);
    ...
    _meeting.on(Events.recordingStopped, () {
      showSnackBarMessage(
          message: "Enregistrement de la réunion arrêté", context: context);
